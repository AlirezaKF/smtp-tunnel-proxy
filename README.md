...
This version **does not implement reverse mode**. The foreign VPS does not dial into the Iran-side client. The current mode is outbound from the client to the VPS, with multiple SOCKS5 connections multiplexed over the same tunnel session.
...
```text
https://github.com/AlirezaKF/smtp-tunnel-proxy
```

The installation commands below use this repository directly.
...
On the Iran-side client server:
...
# Fresh Install on the Foreign VPS, Server Role
...
curl -fsSL https://raw.githubusercontent.com/AlirezaKF/smtp-tunnel-proxy/main/scripts/bootstrap.sh | sudo bash -s -- \
  ...
  --ref main \
  --hostname smtp-v1.example.com \
  --listen-port 587 \
  --ehlo-name smtp-v1.example.com \
...
with your real VPS domain, for example:

```text
smtp-v1.vividaichat.site
```
...
public hostname/domain: smtp-v1.vividaichat.site
...
After installation, create a new user:

```bash
cd /opt/smtp-tunnel
sudo ./smtp-tunnel-adduser client2 --output-dir /root
...
```text
/root/smtp-tunnel-client-client2.tar.gz
```
...
```bash
sudo smtp-tunnel-deluser client2
sudo systemctl restart smtp-tunnel
```
...
# Copy the Client Package to the Iran-Side Server

On the VPS:

```bash
scp /root/smtp-tunnel-client-client1.tar.gz root@IRAN_SERVER_IP:/root/
```

Replace `IRAN_SERVER_IP` with the Iran-side server IP address.
...
# Fresh Install on the Iran-Side Server, Client Role
...
```bash
curl -fsSL https://raw.githubusercontent.com/AlirezaKF/smtp-tunnel-proxy/main/scripts/bootstrap.sh -o /root/bootstrap.sh

sudo bash /root/bootstrap.sh \
  --role client \
  --repo AlirezaKF/smtp-tunnel-proxy \
  --ref main \
  --from-package /root/smtp-tunnel-client-client1.tar.gz
```

If your package has another name, use that filename:
...
```text
/var/backups/smtp-tunnel/20260528-222159/
```
...
If nothing is printed, the server is not listening.
...
If nothing is printed, the client SOCKS listener is not running.
...
If they do not exist, generate certificates:
...
Make sure the username and secret inside the client package match `/etc/smtp-tunnel/users.yaml`.
...
```bash
tar -tzf /root/smtp-tunnel-client-client2.tar.gz | head -50
```

The package should include config and CA files.
